...
def get_sides(region, board)
  val = board[region.first[1]][region.first[0]]

  total = 0

  # top
  uncapped = region.select do |(x, y)|
    target = [x, y-1]
    # right = [x+1, y]
    (!is_valid?(target, board) || board[target[1]][target[0]] != val)
    # (!is_valid?(right, board) || board[right[1]][right[0]] != val)

    # above = [x, y-1]
  end

  uncapped.each do |(x, y)|
    # right
    target = [x+1, y]
    if !uncapped.member?(target)
      total += 1
    end
  end
